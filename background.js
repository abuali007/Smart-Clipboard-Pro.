...
const LICENSE_STATE_HMAC_SECRET = 'smart-clipboard-license-state-v1';
...
async function signLicenseState(licenseInfo, deviceFingerprint) {
    ...
    const keyMaterial = new TextEncoder().encode(`${LICENSE_STATE_HMAC_SECRET}:${deviceFingerprint}`);
...
async function getLicenseCryptoKey(deviceFingerprint) {
    const material = new TextEncoder().encode(`${LICENSE_STATE_HMAC_SECRET}:${deviceFingerprint}`);
...
async function addToClipboardHistory(text) {
    ...
    try {
        ...
        if (useIndexedDB) {
            ...
            history = history.filter(item => (item.text || '').trim() !== text.trim());
...
function isLicenseApiConfigured() {
    return Boolean(LICENSE_API_BASE_URL && LICENSE_API_BASE_URL.startsWith('http'));
}
...
async function callLicenseApi(path, payload = {}, options = {}) {
    ...
    const url = `${LICENSE_API_BASE_URL}${path}`;
...
async function verifyLicenseWithBackend(licenseKey, deviceFingerprint, options = {}) {
    ...
    try {
        const response = await callLicenseApi(LICENSE_API_VERIFY_PATH, payload, options);
...
async function syncLicenseWithServer(currentLicenseInfo = null, forceSubscriptionCheck = false) {
    ...
    try {
        const deviceFingerprint = await getDeviceFingerprint();
        const response = await callLicenseApi(LICENSE_API_SYNC_PATH, {
...
async function acquireActivationLock(timeoutMs = ACTIVATION_LOCK_TIMEOUT_MS) {
    ...
    while (Date.now() < deadline) {
        ...
        if (expired) {
            ...
            if (verify[ACTIVATION_REGISTRY_LOCK_KEY]?.token === token) {
                return token;
            }
        }
...
async function releaseActivationLock(token) {
    ...
    try {
        ...
        if (current[ACTIVATION_REGISTRY_LOCK_KEY]?.token === token) {
...
async function updateClipboardItem(itemId, payload = {}, type = 'history') {
    try {
        ...
        const primaryIndex = primaryList.findIndex((item) => item.id === itemId);
        const secondaryIndex = secondaryList.findIndex((item) => item.id === itemId);
