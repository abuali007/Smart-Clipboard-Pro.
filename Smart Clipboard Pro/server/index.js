...
const ALLOWED_LEMONSQUEEZY_PATHS = new Set(['/licenses/activate', '/licenses/validate']);
...
async function callLemonSqueezy(path, payload = {}) {
    ...
    if (!ALLOWED_LEMONSQUEEZY_PATHS.has(apiPath)) {
        throw new Error('Unsupported Lemon Squeezy API path');
    }
    const response = await fetch(`${LEMONSQUEEZY_API_BASE}${apiPath}`, {
        method: 'POST',
        headers: {
