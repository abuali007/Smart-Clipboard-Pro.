...
function normalizeLicenseKey(value = '') {
    ...
    if (!normalized || !LICENSE_PATTERN.test(normalized)) return null;
    return normalized;
}
...
async function callLemonSqueezy(path, payload = {}) {
...
function mapLemonSqueezyResponseToLicense(data, context = {}) {
    ...
    return {
        ...
        nextChargeAt: normalizeTimestamp(meta.renews_at || attributes.subscription_renews_at || attributes.renews_at),
        expiresAt: normalizeTimestamp(meta.expires_at || meta.subscription_ends_at || attributes.expires_at || attributes.subscription_ends_at),
...
app.post('/api/verify-license', limiter, async (req, res) => {
    ...
    if (!normalized) {
        return res.status(400).json({ valid: false, error: 'Invalid license format. Use XXXX-XXXX-XXXX-XXXX' });
    }
    ...
    try {
        ...
        const data = await callLemonSqueezy(shouldIncrement ? '/licenses/activate' : '/licenses/validate', payload);
