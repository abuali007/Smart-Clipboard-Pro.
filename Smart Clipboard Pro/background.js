...
const LICENSE_API_ACTIONS = new Set(Object.keys(LICENSE_API_ENDPOINTS));
...
function buildLicenseApiUrlFromAction(action) {
    if (!LICENSE_API_ACTIONS.has(action)) {
...
async function callLicenseApi(action, payload = {}, options = {}) {
    ...
    let response;
    try {
        response = await fetch(buildLicenseApiUrlFromAction(action), {
            method: 'POST',
            headers: {
