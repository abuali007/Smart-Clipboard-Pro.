// Smart Clipboard Pro Enhanced - Background Script v5.1.5
// ====================================================

// Constants
const MAX_HISTORY_ITEMS_FREE = Number.MAX_SAFE_INTEGER; // Free users: unlimited history
const MAX_HISTORY_ITEMS_PRO = Number.MAX_SAFE_INTEGER; // Pro users: unlimited history
const MAX_PINNED_ITEMS_FREE = 10; // Free users: 10 pins
const MAX_PINNED_ITEMS_PRO = Number.MAX_SAFE_INTEGER; // Pro users: unlimited pins
const MAX_SNIPPETS_FREE = 10; // Free users: 10 snippets
const MAX_SNIPPETS_PRO = Number.MAX_SAFE_INTEGER; // Pro users: unlimited snippets
...
const LEMONSQUEEZY_VALIDATE_URL = `${LEMONSQUEEZY_API_BASE}/licenses/validate`;
const LEMONSQUEEZY_ACTIVATE_URL = `${LEMONSQUEEZY_API_BASE}/licenses/activate`;
const LEMONSQUEEZY_API_KEY = ''; // Inject via build or runtime storage; prefer backend server to keep secrets out of the client
const LICENSE_API_BASE_URL = 'https://subscription-smart-clipboard-pro.onrender.com'; // Update to your deployed license server base URL
...
chrome.runtime.onInstalled.addListener(async (details) => {
    console.log('Smart Clipboard Pro Enhanced v5.1.5 - Installed/Updated');
...
chrome.runtime.onStartup.addListener(async () => {
    console.log('Smart Clipboard Pro Enhanced v5.1.5 - Startup');
    createContextMenus();
    startClipboardMonitoring();
    await bootstrapLicenseFromSync();
    scheduleLicenseRefreshAlarm();
...
function isLicenseApiConfigured() {
    return Boolean(
        ...
        (LICENSE_API_BASE_URL.startsWith('https://') || LICENSE_API_BASE_URL.startsWith('http://localhost'))
    );
}
...
async function callLicenseApi(action, payload = {}, options = {}) {
    let path;
    switch (action) {
        case 'verify':
            path = LICENSE_API_ENDPOINTS.verify;
            break;
        case 'sync':
            path = LICENSE_API_ENDPOINTS.sync;
            break;
        default:
            throw new Error('License API action is not allowed');
    }
...
async function verifyLicenseWithBackend(licenseKey, deviceFingerprint, options = {}) {
    ...
    try {
        const response = await callLicenseApi('verify', payload, options);
        ...
        const baseInfo = response.licenseInfo || {};
        const licenseInfo = sanitizeLicenseInfo({
            ...baseInfo,
            key: licenseKey,
            activated: true,
            status: baseInfo.status || response.subscriptionStatus || response.status || 'active',
            source: 'backend',
            productId: baseInfo.productId || response.productId || null,
            productName: baseInfo.productName || response.productName || 'Smart Clipboard Pro',
            purchaser: baseInfo.purchaser || {
                email: baseInfo.email || response.email || response.purchaser?.email || '',
                name: baseInfo.name || response.name || response.purchaser?.name || ''
            },
            activatedAt: normalizeTimestamp(baseInfo.activatedAt || response.activatedAt) || Date.now(),
            lastVerifiedAt: Date.now(),
            nextChargeAt: normalizeTimestamp(baseInfo.nextChargeAt || response.nextChargeAt),
            expiresAt: normalizeTimestamp(baseInfo.expiresAt || response.expiresAt),
            subscriptionType: baseInfo.subscriptionType || response.subscriptionType || response.plan || null,
            planLabel: baseInfo.planLabel || derivePlanLabelFromResponse(response),
            usageCount: Number(baseInfo.usageCount ?? response.usageCount ?? response.activations ?? 0),
            maxActivations: Number(baseInfo.maxActivations ?? response.maxActivations ?? response.activationLimit ?? MAX_LICENSE_ACTIVATIONS),
            subscriptionCancelled: Boolean(
                baseInfo.subscriptionCancelled ||
                response.subscriptionStatus === 'canceled' ||
                response.subscriptionStatus === 'cancelled' ||
                response.subscriptionCancelled
            ),
            subscriptionFailedAt: normalizeTimestamp(baseInfo.subscriptionFailedAt || response.subscriptionFailedAt),
            subscriptionId: baseInfo.subscriptionId || response.subscriptionId || null,
            saleId: baseInfo.saleId || response.saleId || null,
            activationId: baseInfo.activationId || response.activationId || null,
            features: Array.isArray(baseInfo.features) && baseInfo.features.length > 0
                ? baseInfo.features
                : (Array.isArray(response.features) && response.features.length > 0 ? response.features : PRO_FEATURES),
            deviceFingerprint
        });
...
async function verifyLicenseViaLemonSqueezy(licenseKey, deviceFingerprint, options = {}) {
    try {
        const response = await requestLemonSqueezyVerification(licenseKey, {
            ...options,
            deviceFingerprint
        });
        return response;
    } catch (error) {
        const friendly = error?.message || 'Lemon Squeezy verification failed';
...
async function verifyLicenseRemotely(licenseKey, deviceFingerprint, options = {}) {
    if (isLicenseApiConfigured()) {
        try {
            ...
            console.warn('Backend verification unreachable, attempting direct Lemon Squeezy call:', error.message || error);
        }
    }
    const lemonResult = await verifyLicenseViaLemonSqueezy(licenseKey, deviceFingerprint, options);
    return { ...lemonResult, source: lemonResult.source || 'lemon-squeezy' };
}
...
async function verifyLicense(licenseKey, options = {}) {
    ...
    try {
        ...
        if (!normalizedKey || !LICENSE_PATTERN.test(normalizedKey)) {
            return { valid: false, error: 'Invalid license key format. Use XXXX-XXXX-XXXX-XXXX' };
        }
        const deviceFingerprint = await getDeviceFingerprint();

        const result = await verifyLicenseRemotely(normalizedKey, deviceFingerprint, { incrementUseCount: false });
        if (!result.success || !result.licenseInfo) {
            if (clearOnFailure) {
                await clearPersistedLicenseInfo();
            }
            return { valid: false, error: result.message || result.error || 'License not recognized for this product' };
        }

        let licenseInfo = sanitizeLicenseInfo({
            ...result.licenseInfo,
            key: normalizedKey,
            deviceFingerprint,
            lastVerifiedAt: Date.now(),
            source: result.source || result.licenseInfo?.source || 'lemon-squeezy'
        });
        const usageCount = Number(licenseInfo.usageCount || 0);
        const activationLimit = Number(licenseInfo.maxActivations || MAX_LICENSE_ACTIVATIONS);
        const status = String(licenseInfo.status || '').toLowerCase();
        if (status && ['disabled', 'revoked', 'inactive'].includes(status)) {
            if (clearOnFailure) {
                await clearPersistedLicenseInfo();
            }
            return { valid: false, error: 'License is inactive or disabled' };
        }
        ...
        if (licenseInfo.expiresAt && licenseInfo.expiresAt <= Date.now()) {
            ...
            return { valid: false, error: licenseInfo.subscriptionCancelled ? 'Subscription has ended' : `License expired on ${endDate}` };
        }
        if (incrementUseCount && isNewActivation) {
            const incrementResult = await verifyLicenseRemotely(normalizedKey, deviceFingerprint, {
                incrementUseCount: true,
                activationId: licenseInfo.activationId
            });
            if (incrementResult.success && incrementResult.licenseInfo) {
                const merged = sanitizeLicenseInfo({
                    ...licenseInfo,
                    ...incrementResult.licenseInfo,
                    source: incrementResult.source || licenseInfo.source
                });
                licenseInfo = merged;
            } else {
                console.warn('Failed to register Lemon Squeezy activation, continuing with previous verification data.');
        ...
        const friendlyMessage = /Lemon Squeezy verification request failed/i.test(error.message || '')
            ? 'Lemon Squeezy servers are temporarily unavailable. Please try again shortly.'
...
async function syncLicenseWithServer(currentLicenseInfo = null, forceSubscriptionCheck = false) {
    ...
    try {
        const deviceFingerprint = await getDeviceFingerprint();
        const response = await callLicenseApi('sync', {
...
function mapLemonSqueezyLicenseResponse(response, options = {}) {
    ...
    const expiresAt = normalizeTimestamp(
        ...
        licenseKeyData.expires_at
    );
    const nextChargeAt = normalizeTimestamp(
        meta.renews_at ||
        meta.subscription_renews_at ||
        attributes.subscription_renews_at ||
    ...
    return {
        key: attributes.key || licenseKeyData.key || options.licenseKey || null,
        activationId: data.id || meta.activation_id || attributes.activation_id || licenseKeyData.activation_id || options.activationId || null,
        activated: meta.activated !== false,
        status,
        source: 'lemon-squeezy',
        productId: attributes.product_id || licenseKeyData.product_id || productRelation?.id || null,
        productName: attributes.product_name || licenseKeyData.product_name || 'Smart Clipboard Pro',
        purchaser,
        activatedAt: normalizeTimestamp(attributes.created_at || meta.activated_at || meta.created_at || licenseKeyData.created_at) || Date.now(),
        lastVerifiedAt: Date.now(),
        nextChargeAt,
        expiresAt,
        subscriptionType: subscriptionType || (nextChargeAt ? 'subscription' : null),
        planLabel: derivePlanLabelFromResponse({
            subscriptionStatus: status,
            subscriptionType: subscriptionType || attributes.subscription_period || attributes.interval
        }),
        usageCount: Number.isFinite(usageCount) && usageCount >= 0 ? usageCount : 0,
        maxActivations: Number.isFinite(activationLimit) && activationLimit > 0 ? activationLimit : MAX_LICENSE_ACTIVATIONS,
...
async function requestLemonSqueezyVerification(licenseKey, options = {}) {
    const {
        incrementUseCount = false,
        deviceFingerprint = null,
        activationId = null,
        instanceName = 'Smart Clipboard Pro'
    } = options;

    if (!LEMONSQUEEZY_API_KEY) {
        throw new Error('Lemon Squeezy API key is not configured. Configure the backend server or inject the API key at build time.');
    }

    const endpoint = incrementUseCount ? LEMONSQUEEZY_ACTIVATE_URL : LEMONSQUEEZY_VALIDATE_URL;
    const body = {
        license_key: licenseKey
    };
    if (deviceFingerprint) {
        body.instance_id = activationId || deviceFingerprint;
    ...
    if (!response.ok) {
        ...
        throw new Error(`Lemon Squeezy verification request failed (${response.status}): ${String(message).slice(0, 140)}`);
    }
    if (payload?.meta?.valid === false) {
        return {
            success: false,
            message: payload?.meta?.error || 'License rejected by Lemon Squeezy',
            licenseInfo: mapLemonSqueezyLicenseResponse(payload, { licenseKey, activationId, deviceFingerprint })
        };
    }

    const mapped = mapLemonSqueezyLicenseResponse(payload, { licenseKey, activationId, deviceFingerprint });
    return {
        success: true,
        licenseInfo: mapped,
        raw: payload,
...
function normalizeLicenseKeyInput(value) {
    ...
    // if already formatted and matches pattern, allow it
...
console.log('Smart Clipboard Pro Enhanced v5.1.5 - Background script loaded');
function validateLicenseKey(licenseKey) {
    const normalized = normalizeLicenseKeyInput(licenseKey);
    if (!normalized || !LICENSE_PATTERN.test(normalized)) {
        return { valid: false, error: 'Invalid license format. Paste the full license key from your receipt.' };
    }
