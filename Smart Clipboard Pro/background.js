...
const LICENSE_API_ALLOWED_PATHS = new Set([LICENSE_API_VERIFY_PATH, LICENSE_API_SYNC_PATH]);
...
const LICENSE_STATE_SECRET_STORAGE_KEY = 'licenseStateSecret';
...
function normalizeLicenseApiPath(path) {
    const trimmed = typeof path === 'string' ? path.trim() : '';
    if (!LICENSE_API_ALLOWED_PATHS.has(trimmed)) {
...
function buildLicenseApiUrl(path) {
    ...
    const normalizedPath = normalizeLicenseApiPath(path);
    return new URL(normalizedPath, LICENSE_API_BASE_URL).toString();
}

async function callLicenseApi(path, payload = {}, options = {}) {
    const url = buildLicenseApiUrl(path);
