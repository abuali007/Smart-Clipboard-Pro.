...
import { getState, patchState, updateState, subscribe } from './state.js';
...
async function bootstrap() {
    try {
        ...
        applyLanguage(normalizedSettings.language);
        patchState({
            history: {
...
function openUpgradePage() {
    const url = LEMON_SQUEEZY_CHECKOUT_URL;
    try {
    ...
    try {
        ...
        console.error('Unable to open upgrade link', error);
    }
}
...
async function toggleAutoSave() {
    ...
    try {
        await saveSettings(updatedSettings);
        showToast(next ? strings.autoToastOn : strings.autoToastOff);
    } catch (error) {
...
async function copyItem(id, type = TABS.HISTORY) {
    ...
    if (normalizedText && normalizedText === lastCopiedSnapshot.text && now - lastCopiedSnapshot.at < 1500) {
        showToast(strings.toastAlreadyCopied);
        return;
    }
    try {
...
async function copySnippet(id) {
    ...
    try {
        ...
        showToast(strings.toastSnippetCopied);
...
async function unpinItem(id) {
    ...
    try {
        ...
        if (response?.success || response === true) {
            ...
            showToast(strings.toastUnpinned);
            refreshTopItems(true);
        }
    } catch (error) {
...
async function deleteItem(id, type = TABS.HISTORY) {
    ...
    try {
        ...
        if (response?.success || response === true) {
            ...
            showToast(strings.toastItemDeleted);
            refreshTopItems(true);
        } else {
...
async function clearHistory() {
    ...
    if (totalItems === 0) {
        showToast(strings.toastHistoryEmpty);
        return;
    }
    ...
    try {
        ...
        if (response?.success || response === true) {
            ...
            showToast(strings.toastHistoryCleared);
            refreshTopItems(true);
        } else {
...
async function saveClipboardEdits() {
    ...
    try {
        ...
        if (response?.success && response.item) {
            ...
            showToast(strings.toastItemUpdated);
            closeClipboardEditModal();
        } else {
...
async function deleteSnippet(id) {
    ...
    try {
        ...
        if (response?.success || response === true) {
            ...
            showToast(strings.toastSnippetDeleted);
        } else {
...
async function saveSnippetFromModal() {
    ...
    try {
        ...
        if (response?.success || response === true) {
            patchState({ snippets: normalizedList });
            closeSnippetModal();
            showToast(strings.toastSnippetSaved);
            if (!isEditing) {
