...
function sanitizeInlineHtml(html = '', allowedTags = ALLOWED_INLINE_TAGS) {
    if (!html || typeof html !== 'string') return document.createDocumentFragment();
    const parser = new DOMParser();
    const doc = parser.parseFromString(`<div>${html}</div>`, 'text/html');
    const container = doc.body.firstChild;
...
export function applyLanguage(langSetting = 'auto') {
    ...
    if (banner) {
        ...
        if (text) {
            const safeBannerContent = sanitizeInlineHtml(currentStrings.bannerText);
            text.replaceChildren(safeBannerContent);
        }
